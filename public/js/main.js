...
$(document).ready(function () {
	...
	// Prevent form submission for guest-review-form1
	$('#guest-review-form1').on('submit', function(e){
		e.preventDefault();
		return false;
	});

	// Handle Next button click for guest review form 1
	$(document).on('click', '#btn_next', function(e){
		...
		if (message == ''){
			...
			$('#error-comments').addClass('d-none');
		}

		// Rating validation
		if (rating == '' || rating == undefined || rating == null){
			$('#error-rating').removeClass('d-none');
			return false;
		} else {
			$('#error-rating').addClass('d-none');
		}
		...
		$.ajax({
			...
			success: function (result) {
				if (result.success){
					...
					// Reset button state for next section (button is now hidden, so no need to reset)
				} else {
					// Reset button state on failure
					$btn.prop('disabled', false).removeClass('disabled');
					$("#btn_spin").addClass('d-none');
					$("#btn-next-text1").text($("#btn-next-text1").data('original-text') || 'Next');
				}
			},
	...
	$('#guest-review-form2').on('submit', function(e){
        ...
        $("#btn-next-text2").text(submitText);
	...
	// Character count for textareas with data-behavior="countable"
	$(document).on('input keyup', 'textarea[data-behavior="countable"]', function() {
		var $textarea = $(this);
		var maxLength = parseInt($textarea.attr('maxlength')) || 500;
		var currentLength = $textarea.val().length;
		var remaining = maxLength - currentLength;
		var $countSpan = $textarea.next('span.float-right');
		
		if ($countSpan.length) {
			// Store original text pattern if not already stored
			if (!$countSpan.data('original-text')) {
				var originalText = $countSpan.text();
				$countSpan.data('original-text', originalText);
			}
			
			// Extract the text pattern (everything after the number)
			var originalText = $countSpan.data('original-text');
			var textPattern = originalText.replace(/^\d+\s*/, ''); // Remove leading number and space
			
			// Update with remaining count
			$countSpan.text(remaining + ' ' + textPattern);
		}
	});
	
	// Initialize count on page load for textareas with existing content
	$('textarea[data-behavior="countable"]').each(function() {
		var $textarea = $(this);
		var maxLength = parseInt($textarea.attr('maxlength')) || 500;
		var currentLength = $textarea.val().length;
		var remaining = maxLength - currentLength;
