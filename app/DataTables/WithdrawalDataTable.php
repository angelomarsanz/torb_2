...
class WithdrawalDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($withdraw) {
                if ($withdraw->status != 'Accepted') {
                    $edit = '<a href="' . url('admin/withdrawals/approve/' . $withdraw->id) . '" class="btn-action btn-action-edit" title="Authorize"><i class="fa fa-check-square-o"></i></a>&nbsp;';
                    return $edit;
                } else {
                    $edit = '&nbsp;';
                    return $edit;
    ...
    {
        return $this->builder()
            ->addColumn(['data' => 'uname', 'name' => 'users.name', 'title' => 'User Name','orderable' => false, 'className' => 'bold-column'])
