...
class BookingsDataTable extends DataTable
{
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('status', function ($bookings) {
                ...
                $statusHtml = '<span class="status-badge ' . $badgeClass . '"><i class="fa ' . $icon . '"></i> ' . $status . '</span>';
                
                if (!in_array($status, ['Accepted', 'Pending'])) {
                    if ($bookings->check_guest_payout == 'yes') {
                        $statusHtml .= "<br/><span class='status-badge status-processing mt-1'><i class='fa fa-reply'></i> Refund</span>";
                    }
                }
                return $statusHtml;
            })
            ->addColumn('action', function ($bookings) {
                return '<a href="' . url('admin/bookings/detail/' . $bookings->id) . '" class="btn-action btn-action-edit" title="Detail View"><i class="fa fa-share"></i></a>&nbsp;';
            })
            ->rawColumns(['host_name', 'guest_name', 'total_amount', 'property_name', 'status', 'action'])
            ->toJson();
    }
    ...
    {
        return $this->builder()
            ->addColumn(['data' => 'id', 'name' => 'bookings.id', 'title' => 'ID', 'visible' => false, 'width' => '5%', 'className' => 'text-center'])
