...
class Currency extends Model
{
    ...
    public function getSessionCodeAttribute()
    {
        if (Session::get('currency')) {
            ...
            $data = self::getAll();
            // Fallback strategy: try multiple options before defaulting to 'USD'
            $defaultCurrency = $data->firstWhere('default', 1);
            if (is_null($defaultCurrency)) {
                $defaultCurrency = $data->where('status','Active')->first();
            }
            if (is_null($defaultCurrency)) {
                $defaultCurrency = $data->first();
            }
