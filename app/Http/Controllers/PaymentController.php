    ...
    public function index(Request $request)
    {
        ...
        if ( !$request->isMethod('post') && ! $checkin) {
            ...
            if ($property && $property->slug) {
                return redirect()->route('property.single', ['slug' => $property->slug]);
            }
            return redirect()->to('properties/' . $request->id);
        }
        ...
        if (((isset($data['price_list']->status) && ! empty($data['price_list']->status)) ? $data['price_list']->status : '') == 'Not available') {
            ...
            if ($data['result'] && $data['result']->slug) {
                return redirect()->route('property.single', ['slug' => $data['result']->slug]);
            }
    ...
    public function createBooking(Request $request)
    {
        ...
        if ($request->payment_method) { 
          return redirect()->route('gateway.pay', ['gateway' => $request->payment_method]);
                ...
                $code = $msg[0];
                $errorMessage = $msg[1];
                // Message removed - the booking/requested page already displays this information
                return redirect()->to('booking/requested?code=' . $code);
    ...
    public function store($data)
    {
       ...
       $booking->date_with_price    = json_encode($allData);
        ...
        if ($booking->status =='Accepted') {
            ...
            if ( !empty(Auth::user()->formatted_phone )) {
                $smsBookingConfirm = twilioSendSms(Auth::user()->formatted_phone, $instantBookingConfirm);
                $smsPaymentConfirm = twilioSendSms(Auth::user()->formatted_phone, $instantBookingPaymentConfirm);
                if (!$smsBookingConfirm || !$smsPaymentConfirm ) {
                    Log::warning('SMS sent failed but process continued', ['phone' => Auth::user()->formatted_phone]);
                }
            }

        } else {
            if ( !empty(Auth::user()->formatted_phone )) {
                $smsBookingConfirm = twilioSendSms(Auth::user()->formatted_phone, __('Your booking is initiated, Wait for confirmation'));
                if (!$smsBookingConfirm ) {
                    Log::warning('SMS sent failed but process continued', ['phone' => Auth::user()->formatted_phone]);
                }
            }
