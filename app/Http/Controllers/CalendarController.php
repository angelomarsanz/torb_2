...
class CalendarController extends Controller
{
    ...
    public function generate($property_id = '', $year = '', $month = '')
    {
        ...
        $out .= '<div class="row-space-2 deselect-on-click">
                    <a href="' . url('manage-listing/' . $property_id . '/calendar') . '" class="month-nav month-nav-previous panel text-center neg-transition-scale" data-year="' . $prvYear . '" data-month="' . $prvMonth . '"> <i class="fa fa-chevron-left fa-lg calendar-icon-style"></i> </a>
                    <a href="' . url('manage-listing/' . $property_id . '/calendar') . '" class="month-nav month-nav-next panel text-center neg-transition-scale" data-year="' . $nxtYear . '" data-month="' . $nxtMonth . '"> <i class="fa fa-chevron-right fa-lg calendar-icon-style"></i> </a>
    ...
    public function calenderPriceSet(CalendarPriceSetRequest $request, CalendarController $calendar)
    {
        ...
        // Format dates based on the specified format
    ...
    public function icalendarImport(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (empty($events) || $ical->event_count == 0) {
                $data['status'] = 0;
                $data['error'] = ['url' => [__('No events found in the calendar URL or invalid calendar format.')]];
                return $data;
            }
