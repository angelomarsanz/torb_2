...
class SearchController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->mapKey = Settings('restrict_api_key') == 'Yes' ? config("vrent.google_geocode_map_key") : config("vrent.google_map_key");
    }
    ...
    public function index(Request $request)
    {
        ...
        $map_where = 'https://maps.google.com/maps/api/geocode/json?key=' . $this->mapKey . '&address=' . $address . '&sensor=false';
        ...
        if (isset($json->{'results'}) && $json->{'results'}) {
            ...
            if (config('vrent.app_demo')) {
                $firstProperty = Properties::where('status', 'Listed')->with('property_address')->first();
                if ($firstProperty && $firstProperty->property_address) {
                    $data['lat'] = $firstProperty->property_address->latitude;
                    $data['long'] = $firstProperty->property_address->longitude;
                } else {
                    $data['lat'] = 40.7128;
                    $data['long'] = -74.0060;
                }
        ...
        if (Session::get('currency'))
            $data['currency_symbol'] = $currency->firstWhere('code', Session::get('currency'))->symbol;
        else
            $data['currency_symbol'] = $currency->firstWhere('default', 1)->symbol;
    ...
    function searchResult(Request $request)
    {
        if (config('vrent.app_demo')) {
            ...
            echo $properties->paginate(Session::get('row_per_page'))->toJson();
            return;
        }

        $full_address = $request->input('location');
        $exploded_address = explode(',', $full_address);
        $city = $exploded_address[0];
        ...
        $map_where = 'https://maps.google.com/maps/api/geocode/json?key=' . $this->mapKey . '&address=' . $address . '&sensor=false&libraries=places';
        $geocode = $this->content_read($map_where);
        $json = json_decode($geocode);
        ...
        if ($data['lat'] != 0) {
            ...
            $properties = Properties::with([
                ...
                ->whereHas('property_address', function ($query) use ($city) {
