...
class IcalendarController extends Controller
{
    ...
    public function __construct($filename)
    {
        ...
        $lines = explode("\n", $lines);
        if (empty($lines) || !isset($lines[0]) || stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
            return false;
        } else {
    ...
    public function curl_get_contents($url)
    {
        ...
        if ($data === false || !empty($error) || $httpCode >= 400) {
            return false;
        }
    ...
    {
        if (!isset($this->cal[$component])) {
            $this->cal[$component] = [];
        }
        
        if ($keyword == false) {
            $keyword = isset($this->last_keyword) ? $this->last_keyword : '';
            switch ($component) {
                case 'VEVENT':
                    ...
                    break;
                case 'VTODO':
                    if (isset($this->cal[$component][$this->todo_count - 1][$keyword])) {
        ...
        if ($keyword && (stristr($keyword, "DTSTART") or stristr($keyword, "DTEND"))) {
        ...
        switch ($component) {
            case "VTODO":
                if ($this->todo_count > 0 && !isset($this->cal[$component][$this->todo_count - 1])) {
                    $this->cal[$component][$this->todo_count - 1] = [];
                }
                ...
                break;
            case "VEVENT":
                if ($this->event_count > 0 && !isset($this->cal[$component][$this->event_count - 1])) {
                    $this->cal[$component][$this->event_count - 1] = [];
                }
    ...
    public function iCalDateToUnixTimestamp($icalDate)
    {
        ...
        if (empty($date) || !isset($date[1]) || $date[1] <= 1970) {
            return false;
        }
       
        $timestamp = mktime(
            isset($date[4]) ? (int)$date[4] : 0,
            isset($date[5]) ? (int)$date[5] : 0,
            isset($date[6]) ? (int)$date[6] : 0,
            (int)$date[2],
            (int)$date[3],
            (int)$date[1]
        );
    ...
    {
        if (!isset($this->cal['VEVENT']) || !is_array($this->cal['VEVENT'])) {
            return [];
        }
