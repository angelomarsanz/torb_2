    ...
    public function apiInformations(Request $request)
    {
        if (! $request->isMethod('post')) {
            ...
            if (settings('restrict_api_key') == 'Yes') {
                $rules['geocode_google_map_key'] = 'required';
                $fieldNames['geocode_google_map_key'] = 'Google Map Geocode Key';
            }
            ...
            if ($validator->fails()) {
                ...
                if (env('APP_MODE', '') != 'test') {
                    ...
                    if (settings('restrict_api_key') == 'Yes') {
                        Settings::where(['name' => 'geocode_key', 'type' => 'googleMap'])->updateOrInsert(['name' => 'geocode_key', 'type' => 'googleMap'], ['value' => $request->geocode_google_map_key]);
                        changeEnvironmentVariable('GOOGLE_MAP_GEOCODE_KEY' , $request->geocode_google_map_key);
                    }
                    event('eloquent.updated: App\Models\Settings', (new Settings()));
