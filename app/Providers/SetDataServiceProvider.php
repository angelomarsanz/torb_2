...
class SetDataServiceProvider extends ServiceProvider
{
    ...
    public function currency()
    {
        ...
        // Fallback strategy for default_currency if still null
        if (!$default_currency) {
            $default_currency = Currency::getAll()->where('status', '=', 'Active')->first();
        }
        if (!$default_currency) {
            $default_currency = Currency::getAll()->first();
        }
        ...
        // Only set session if default_currency is not null
        if ($default_currency) {
            ...
            $currencies = collect([]);
            View::share('currencies', $currencies);
            View::share('currency', collect(['USD' => 'USD']));
            
            $default_currency = (object)[];
            $default_currency->code = 'USD';
            $default_currency->symbol = '$';
            $default_currency->name = 'US Dollar';
