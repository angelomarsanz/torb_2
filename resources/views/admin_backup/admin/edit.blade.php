						...
						<form id="edit_admin" method="post" action="{{ url('admin/edit-admin/' . $result->id) }}" class="form-horizontal" enctype="multipart/form-data">
							...
							<div class="card-body">
								...
								<div class="row mb-3">
									...
									<div class="col-md-5">
										<input type="text" name="email" class="form-control f-14" id="email" placeholder="Email" value="{{ $result->email }}">
										<span class="text-danger f-12">{{ $errors->first("email") }}</span>
									</div>
								</div>

								<div class="row mb-3">
									<label for="password" class="col-md-3 col-form-label text-md-end fw-bold">Password</label>
									<div class="col-md-5">
									...
									<div class="col-md-4">
										<small class="text-muted f-12">Enter new password only. Leave blank to use existing password.</small>
									</div>
								</div>
