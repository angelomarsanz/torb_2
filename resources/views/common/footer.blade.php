                ...
                <div class="row mt-3">
                    <div class="col-md-12">
                        <p class="text-center text-underline">
                            <a href="javascript:void(0)" aria-label="modalLanguge" data-toggle="modal" data-target="#languageModalCenter"> <i class="fa fa-globe"></i> {{ Session::get('language_name')  ?? (isset($default_language) && $default_language ? $default_language->name : 'English') }} </a>
                            <a href="javascript:void(0)" aria-label="modalCurrency" data-toggle="modal" data-target="#currencyModalCenter"> <span class="ml-4">{!! Session::get('symbol')  !!} - <u>{{ Session::get('currency')  }}</u> </span></a>
                        </div>
                    </div>
                </div>
        </div>
    ...
    <div class="modal fade mt-5 z-index-high" id="currencyModalCenter" tabindex="-1" role="dialog" aria-labelledby="languageModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body pb-5">
					<div class="row">
						@if(isset($currencies) && $currencies && $currencies->count() > 0)
							@foreach ($currencies as $key => $value)
							<div class="col-6 col-sm-3 p-3">
                                <div class="currency px-3 text-16 {{ (Session::get('currency') == $value->code) ? 'border border-success rounded-5 currency-active' : '' }}">
						...
						@else
							<div class="col-6 col-sm-3 p-3">
								<div class="currency px-3 text-16 border border-success rounded-5 currency-active">
